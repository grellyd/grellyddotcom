...
func run() error {
	...
	if config.TLS {
		...
		go func() {
			...
			err = http.ListenAndServe(ChallengePort, certManager.HTTPHandler(nil))
			if err != nil {
		...
		globallogger.Info(fmt.Sprintf("Serving on %s", TLSPort))
		if err = server.ListenAndServeTLS("", ""); err != nil {
			return errors.Wrap(err, "failed to ListenAndServeTLS")
		}

	} else {
		globallogger.Info(fmt.Sprintf("Serving on %s", LocalPort))
